...
if (isActionAccessible($guid, $connection2, '/modules/Admissions/studentEnrolment_manage_bulkAdd.php') == false) {
    ...
    $page->return->addReturns(
        ['error33' => __('Unable to generate a StudentID'),
        'error12' => __('Some of the selected students does not have a valid status. Expected \'Full\' or \'Expected\''),
        'error13' => __('Some of the selected students are already enrolled'),
    ]);
        
    if (isset($_GET['invalidIDs'])) {
        echo "<h3>".__("Invalid Users")."</h3>";
        // Retrieve invalid student IDs from the GET parameter and convert to an array
        $invalidIDs = explode(',', $_GET['invalidIDs']);
        
        // Fetch user details using the getUsers method
        $users = $container->get(UserGateway::class)
                ->getUsers($invalidIDs);
    
        // Display the user details in a table
        if (!empty($users)) {
            echo "<table border='1'>";
            echo "<tr><th>".__("Student ID")."</th><th>".__("First Name")."</th><th>".__("Surname")."</th></tr>";
    ...
    if (isset($_GET['alreadyEnrolled'])) {
        echo "<h3>".__("Invalid Users")."</h3>";
        // Retrieve invalid student IDs from the GET parameter and convert to an array
        $alreadyEnrolled = explode(',', $_GET['alreadyEnrolled']);
        
        // Fetch user details using the getUsers method
        $users = $container->get(UserGateway::class)
                ->getUsers($alreadyEnrolled);
    ...
    if ($gibbonSchoolYearID == '') {
        ...
        $form = Form::create('studentEnrolmentBulkAdd', $session->get('absoluteURL').'/modules/'.$session->get('module')."/studentEnrolment_manage_bulkAddProcess.php?gibbonSchoolYearID=$gibbonSchoolYearID&search=$search");
        ...
        $row = $form->addRow();
           $row->addLabel('gibbonPersonIDs', __('Students'))->description(__('Only includes students not enrolled in specified year.'));
           $row->addSelect('gibbonPersonIDs')->fromResults($students)->required()->selectMultiple();
